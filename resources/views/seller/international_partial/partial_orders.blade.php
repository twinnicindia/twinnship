<!--<input type="hidden" value="{{$count_ajax_order ?? 0}}" id="total_ajax">-->
<input type="hidden" value="{{$order->total() ?? 0}}" id="total_ajax">
@if(count($order)!=0)
@php
    $orderStatus = array(
    "pending" => "Pending",
...
<tr id="row{{$o->id}}">
    ...
    <td>{{$o->channel==''?"Custom" : "$o->channel"}}<br>{{$o->seller_channel_name ?? '-'}}</td>
    ...
    @else
        <td><span class="{{$o->invoice_amount == '' ? 'text-danger font-weight-bold' : ''}}">Amount </span> : <span id="displayInvoiceAmount_{{$o->id}}">{{$o->invoice_amount}}</span>
            @if($o->status == 'pending')
                <a class="editInvoice ml-1" id="editInvoice_{{$o->id}}" data-amount="{{$o->invoice_amount}}" data-type="all_order" data-placement="top" data-toggle="tooltip" data-id="{{$o->id}}"> <i class="fas fa-edit fa-lg text-primary"></i></a>
            @endif
    ...
    @else
        <td>
            <span class="{{$o->s_customer_name == '' ? 'text-danger font-weight-bold' : ''}}">Name</span> :{{strlen($o->b_customer_name) > 15 ? substr("$o->b_customer_name",0,15)."..." : $o->b_customer_name}} @if(strlen($o->b_customer_name) > 15)<a href="javascript:;" class=" mx-0" data-placement="top" data-toggle="tooltip" data-html="true" data-original-title="{{$o->b_customer_name}}"><i class="fas fa-eye text-primary"></i></a>@endif<br>
            <span class="{{$o->s_contact == '' ? 'text-danger font-weight-bold' : ''}}">Contact</span> :{{$o->b_contact}}
        </td>
    @endif
    @if($o->o_type == 'forward')
        ...
        @else
            <td id="all_order_delivery_{{$o->id}}">{{$o->s_state}} <br>
                {{$o->s_city}} <br>
                {{$o->s_pincode}}  <a href="javascript:;" class=" mx-0" data-placement="top" data-html="true" data-toggle="tooltip" data-original-title="{{$o->s_address_line1}} <br> {{$o->s_address_line2}} <br> {{$o->s_city}} {{$o->s_state}} {{$o->s_pincode}}"><i class="fas fa-eye text-primary"></i> @if($o->status == 'pending')<i data-id="{{$o->id}}" data-name="{{$o->s_customer_name}}" data-contact="{{$o->s_contact}}" data-address1="{{$o->s_address_line1}}" data-address2="{{$o->s_address_line2}}" data-pincode="{{$o->s_pincode}}" data-city="{{$o->s_city}}" data-state="{{$o->s_state}}" data-country="{{$o->s_country}}" class="fas fa-edit text-primary editDeliveryAddress"></i>@endif</a>
            </td>
        @endif
    @else
        @if(env('AMAZON_PII_ARCHIVE') == true && in_array(strtolower($o->channel), ['amazon', 'amazon_direct']) && now()->parse($o->inserted)->lte(now()->subDays(env('AMAZON_PII_ARCHIVE_DAY'))))
    ...
    <td class="OD-{{$o->id}}">
        <div>
            ...
            <span class="{{$o->length == '' ? 'text-danger font-weight-bold' : ''}}">{{$o->length !='' ? $o->length : '__'}}</span> * <span class="{{$o->breadth == '' ? 'text-danger font-weight-bold' : ''}}">{{$o->breadth !='' ? $o->breadth : '__'}}</span> * <span class="{{$o->height == '' ? 'text-danger font-weight-bold' : ''}}">{{$o->height !='' ? $o->height : '__'}}</span><br>
    ...
    <td>
    @if($o->status == 'pending')
        @if(session('filter_status') != 'unprocessable_order_data')
        <button type="button" class="btn btn-primary internationalShipOrderButton btn-sm mx-0" title="Ship Order" data-id="{{$o->id}}" data-status="{{$o->status}}" data-placement="top" data-toggle="tooltip" data-original-title="Ship Order"><i class="fas fa-shipping-fast"></i></button>
        ...
        <button type="button" title="Clone Order" class="btn btn-warning clone_data btn-sm mx-0" data-number="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Clone Order"> <i class="fas fa-clone"></i> </button>
        <button type="button" title="Reverse Order" class="btn btn-info reverse_data btn-sm mx-0" data-number="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Reverse Order"> <i class="fas fa-undo-alt"></i> </button>
        @endif
    ...
    @elseif ($o->status == 'delivered')
        ...
        <button type="button" title="Clone Order" class="btn btn-warning clone_data btn-sm mx-0" data-number="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Clone Order"> <i class="fas fa-clone"></i> </button>
        <button type="button" title="Reverse Order" class="btn btn-info reverse_data btn-sm mx-0" data-number="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Reverse Order"> <i class="fas fa-undo-alt"></i> </button>
    @elseif ($o->status == 'cancelled' || $o->status == 'rto_initated' || $o->status == 'rto_initiated')
        <button type="button" title="Clone Order" class="btn btn-warning clone_data btn-sm mx-0" data-number="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Clone Order"> <i class="fas fa-clone"></i> </button>
    @else
        <a title="Download Invoice" href='{{("single_order/invoice/pdf/$o->id")}}' class="btn btn-primary btn-sm mx-0" data-placement="top" data-toggle="tooltip" data-original-title="Print Order"><i class="fa fa-print"></i></a>
        <a title="Download Label" href='{{("single_order/lable/pdf/$o->id")}}' class="btn btn-info btn-sm mx-0" data-placement="top" data-toggle="tooltip" data-original-title="Print Order"><i class="fa fa-tag"></i></a>
