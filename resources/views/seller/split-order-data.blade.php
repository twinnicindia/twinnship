...
@forelse($order as $o)
    <tr id="row{{$o->id}}">
        ...
        @else
            <td>
                <span class="{{$o->s_customer_name == '' ? 'text-danger font-weight-bold' : ''}}">Name</span> :{{strlen($o->s_customer_name) > 15 ? substr("$o->b_customer_name",0,15)."..." : $o->b_customer_name}} @if(strlen($o->b_customer_name) > 15)<a href="javascript:;" class=" mx-0" data-placement="top" data-toggle="tooltip" data-html="true" data-original-title="{{$o->b_customer_name}}"><i class="fas fa-eye text-primary"></i></a>@endif<br>
                <span class="{{$o->b_contact == '' ? 'text-danger font-weight-bold' : ''}}">Contact</span> :{{$o->s_contact}}
            </td>
        @endif
        @if($o->o_type == 'forward')
        ...
        <td>
            <button type="button" title="Split Order" class="btn btn-primary split_order btn-sm mx-0" data-id="{{$o->id}}" data-placement="top" data-toggle="tooltip" data-original-title="Split Order"> <i class="fa fa-page-break"></i> </button>
        </td>
    </tr>
@empty
    <tr>
