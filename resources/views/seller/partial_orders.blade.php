...
@endphp
@foreach($order as $o)
    @if($o->rto_status == 'y' && $o->status == 'delivered')
        @php($o->status = 'rto_delivered')
    @elseif($o->rto_status == 'y' && $o->status == 'in_transit')
        @php($o->status = 'rto_in_transit')
    @endif
<tr id="row{{$o->id}}" data-sku="{{$o->product_sku}}">
    ...
    <td><a href='{{url("/view-order/$o->id")}}' target="_blank">{{$o->customer_order_number}}</a>@if($o->is_tagged == 'y' && $o->channel == 'shopify')&nbsp;&nbsp;<a class="shopify_tags" data-id="{{$o->id}}"><i class="fa fa-tag"></i></a>@endif<br>
        <span class="{{$o->status=='pending' ? 'text-danger' : 'text-primary'}} font-weight-bold text-capitalize">{{$orderStatus[$o->status] ?? "NA"}}</span>
        <br>
        <span class="badge {{$o->o_type =='forward'?'badge-success':'badge-danger'}}">{{$o->o_type}}</span>&nbsp;&nbsp;
        @if($o->is_qc == 'y')
            <a href="javascript:;" title="QC Information" data-toggle="tooltip" class="qc_information" data-id="{{$o->id}}"><i class="fas fa-eye text-primary"></i></a>
        @endif
        <br>
    </td>
