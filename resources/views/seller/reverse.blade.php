<input type="hidden" value="{{count($order)}}" id="total_reverse_order">
@php
    $orderStatus = array(
    "pending" => "Pending",
    "shipped" => "Shipped",
...
<tr id="row{{$o->id}}">
    ...
    <td><a href='{{url("/view-order/$o->id")}}' target="_blank">{{$o->customer_order_number}}</a>@if($o->is_tagged == 'y' && $o->channel == 'shopify')&nbsp;&nbsp;<a class="shopify_tags" data-id="{{$o->id}}"><i class="fa fa-tag"></i></a>@endif<br>
        <span class="{{$o->status=='pending' ? 'text-danger' : 'text-primary'}} font-weight-bold text-capitalize">{{$o->status}}</span>
        <br>
    ...
    <td>
        <div>
            <span class="{{$o->weight == '' ? 'text-danger font-weight-bold' : ''}}">Wt</span> : {{$o->weight != '' ? $o->weight / 1000 : ''}}
            @if($o->status == 'pending')
