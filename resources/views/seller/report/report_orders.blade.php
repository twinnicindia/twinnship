        ...
        <tr>
            ...
            <td>
                ...
                @else
                    ...
                    $qty = \App\Models\Product::where('order_id', $o->id)->orWhere('order_id', $o->order_id ?? "")->sum('product_qty');
                    ?>
                    ...
                    Qty : {{$qty}} &nbsp;<a href="javascript:;" class=" mx-0" data-placement="top" data-toggle="tooltip" data-html="true" data-original-title="Name : @foreach(explode(',', $o->product_name) as $name) {{$name}} @endforeach <br> SKU : @foreach(explode(',', $o->product_sku) as $sku) {{$sku}} @endforeach <br> QTY : {{$qty}}"><i class="fas fa-eye text-primary"></i></a>
                @endif
            </td>
           @if(env('AMAZON_PII_ARCHIVE') == true && in_array(strtolower($o->channel), ['amazon', 'amazon_direct']) && now()->parse($o->inserted)->lte(now()->subDays(env('AMAZON_PII_ARCHIVE_DAY'))))
                 <td>
            ...
            <td>
                @if($o->status != "pending")
                    ...
                    AWB : <a href='{{url("track-order/$o->awb_number")}}' target="_blank">{{$o->awb_number}}</a>
                @endif
            </td>
        </tr>
    @endforeach
