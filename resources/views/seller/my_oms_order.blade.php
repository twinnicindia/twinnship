...
<tr id="row{{$o->id}}">
    ...
    <td><a href='{{url("/view-order/$o->id")}}' target="_blank">{{$o->customer_order_number}}</a><br>
        <span class="{{$o->status=='pending' ? 'text-danger' : 'text-primary'}} font-weight-bold text-capitalize">{{$o->rto_status=='y'?'RTO ':''}}{{$o->status}}</span>
        <br>
    ...
    <td>
        ...
        @if($o->courier_partner)
        <a title="Download Label" href='{{url("my-oms/single_order/lable/pdf/$o->id")}}' class="btn btn-info btn-sm mx-0" data-placement="top" data-toggle="tooltip" data-original-title="Print Order"><i class="fa fa-tag"></i></a>
        @endif
    </td>
</tr>
@empty
