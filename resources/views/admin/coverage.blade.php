...
<head>
    ...
    <title> Coverage Management | {{env('appTitle')}} </title>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Manage Coverage</h3>
                            </div>
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" id="quickForm" action="{{route('add_coverage')}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="link">Coverage Link</label>
                                                <input type="text" name="link" class="form-control" id="link" placeholder="Link of Brand">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">All Coverage</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <button type="button" id="addDataButton" class="btn btn-warning"><i class="fa fa-plus"></i> Add Coverage</button><br><br>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#addDataButton').click(function () {
            $('#quickForm').prop('action','{{route('add_coverage')}}');
            showForm();
        });
        ...
        $('#example1').on('click','.remove_data',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    showOverlay();
                    $.ajax({
                        url : '{{url('/')."/administrator/delete-coverage"}}/'+that.data('id'),
        ...
        $('#example1').on('change','.change_status',function(){
            ...
            $.ajax({
                ...
                url : '{{url('/')."/administrator/coverage-status"}}',
                success: function (response) {
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            var that=$(this);
            $.ajax({
                url : '{{url('/')."/administrator/modify-coverage/"}}' + that.data('id'),
                success: function (response) {
                    var info=JSON.parse(response);
                    $('#quickForm').prop('action','{{route('update_coverage')}}');
                    $('#hid').val(info.id);
