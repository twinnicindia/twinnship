...
<head>
    ...
    <title> Plan Features Management | {{env('appTitle')}} </title>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Manage Features</h3>
                            </div>
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" id="quickForm" action="{{route('add_features')}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title" class="form-control" id="title" placeholder="Title of feature">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="detail">Feature Detail</label>
                                                <input type="text" name="detail" class="form-control" id="detail" placeholder="Detail of feature">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="image">Choose Image (302 * 220)</label>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <!-- /.card-header -->
                            <div class="card-body">
                                <button type="button" id="addDataButton" class="btn btn-warning"><i class="fa fa-plus"></i> Add Feature</button><br><br>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#addDataButton').click(function () {
            $('#quickForm').prop('action','{{route('add_features')}}');
            showForm();
        });
        ...
        $('#example1').on('click','.remove_data',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    showOverlay();
                    $.ajax({
                        url : '{{url('/')."/administrator/delete-features"}}/'+that.data('id'),
        ...
        $('#example1').on('change','.change_status',function(){
            ...
            $.ajax({
                ...
                url : '{{url('/')."/administrator/features-status"}}',
                success: function (response) {
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            var that=$(this);
            $.ajax({
                url : '{{url('/')."/administrator/modify-features/"}}' + that.data('id'),
                success: function (response) {
                    var info=JSON.parse(response);
                    $('#quickForm').prop('action','{{route('update_features')}}');
                    $('#hid').val(info.id);
