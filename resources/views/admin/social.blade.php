...
<head>
    ...
    <title> Social Links Management | {{env('appTitle')}} </title>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" id="quickForm" action="{{route('add_social')}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="image">Choose Image (32 x 32)</label>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <!-- /.card-header -->
                            <div class="card-body">
                                <button type="button" id="addDataButton" class="btn btn-warning"><i class="fa fa-plus"></i> Add Link</button><br><br>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @foreach($links as $a)
                                            <tr id="row{{$a->id}}">
                                                ...
                                                <td><a target="_blank" href="{{$a->link}}">Click to See</a></td>
                                                <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#addDataButton').click(function () {
            $('#quickForm').prop('action','{{route('add_social')}}');
            showForm();
        });
        ...
        $('#example1').on('click','.remove_data',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    showOverlay();
                    $.ajax({
                        url : '{{url('/')."/administrator/delete-social"}}/'+that.data('id'),
        ...
        $('#example1').on('change','.change_status',function(){
            ...
            $.ajax({
                ...
                url : '{{url('/')."/administrator/social-status"}}',
                success: function (response) {
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            var that=$(this);
            $.ajax({
                url : '{{url('/')."/administrator/modify-social/"}}' + that.data('id'),
                success: function (response) {
                    var info=JSON.parse(response);
                    $('#quickForm').prop('action','{{route('update_social')}}');
                    $('#hid').val(info.id);
                    $('#icon').val(info.icon);
                    $('#link').val(info.link);
                    showForm();
