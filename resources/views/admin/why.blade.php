...
<head>
    ...
    <title> Why Choose Management | {{env('appTitle')}} </title>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Manage Why Choose</h3>
                            </div>
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" id="quickForm" action="{{route('add_why')}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title" class="form-control" id="title" placeholder="Title of Why Choose">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="detail">Feature Detail</label>
                                                <textarea type="text" name="detail" class="form-control" id="detail" placeholder="Detail of Why Choose" rows="5"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="image">Choose Image (494 * 496)</label>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">All Why Choose</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
    ...
    $(document).ready(function () {
        ...
        $('#example1').on('click','.remove_data',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    showOverlay();
                    $.ajax({
                        url : '{{url('/')."/administrator/delete-why"}}/'+that.data('id'),
        ...
        $('#example1').on('change','.change_status',function(){
            ...
            $.ajax({
                ...
                url : '{{url('/')."/administrator/why-status"}}',
                success: function (response) {
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            var that=$(this);
            $.ajax({
                url : '{{url('/')."/administrator/modify-why/"}}' + that.data('id'),
                success: function (response) {
                    var info=JSON.parse(response);
                    $('#quickForm').prop('action','{{route('update_why')}}');
                    $('#hid').val(info.id);
                    $('#title').val(info.title);
                    if(info.detail!==null)
                        tinymce.get('detail').setContent(info.detail)
        ...
        $('#quickForm').validate({
            ...
            messages: {
                title: {
                    required: "Please enter title of Why Choose",
                },
                detail: {
                    required: "Please enter detail of Why Choose",
                }
            },
            errorElement: 'span',
