                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" id="quickForm" action="{{route('add_about')}}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <!-- /.card-header -->
                            <div class="card-body">
                                <button type="button" id="addDataButton" class="btn btn-warning"><i class="fa fa-plus"></i> Add About</button><br><br>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#addDataButton').click(function () {
            $('#quickForm').prop('action','{{route('add_about')}}');
            showForm();
        });
        ...
        $('#example1').on('click','.remove_data',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    showOverlay();
                    $.ajax({
                        url : '{{url('/')."/administrator/delete-about"}}/'+that.data('id'),
        ...
        $('#example1').on('change','.change_status',function(){
            ...
            $.ajax({
                ...
                url : '{{url('/')."/administrator/about-status"}}',
                success: function (response) {
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            var that=$(this);
            $.ajax({
                url : '{{url('/')."/administrator/modify-about/"}}' + that.data('id'),
                success: function (response) {
                    var info=JSON.parse(response);
                    $('#quickForm').prop('action','{{route('update_about')}}');
                    $('#hid').val(info.id);
                    $('#name').val(info.name);
                    $('#post').val(info.post);
                    $('#link').val(info.link);
